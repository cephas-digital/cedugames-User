...
export default function LearnLevelMap({ levels, programId }) {
  ...
  return <section className="learn-level-map-wrap">
    ...
    <div className="learn-level-start"><button disabled={!selected} onClick={() => navigate(`/learn?program=${programId}&parent=${selected.id}&type=questions`)} className="home-start-button min-h-12 w-full max-w-sm rounded-2xl bg-purple-600 px-5 py-3 text-base font-black text-white shadow-xl disabled:opacity-50">{selected ? `Start ${selected.title}` : "Select a level"}</button></div>
  </section>;
}
