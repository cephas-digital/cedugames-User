...
export default function OptionsGrid({ options, onSelect, selectedOption, revealedOptionId }) {
  ...
  return <div className="w-full max-w-3xl mx-auto">
    ...
    <ul className={`grid gap-3 sm:gap-6 mt-4 sm:mt-6 ${useTwoColumns?"grid-cols-2":"grid-cols-1"}`}>
      {options.map((option) => <li key={option.id} className={`min-w-0 rounded-2xl overflow-hidden bg-white/90 shadow-md transition ${selectedOption?.id===option.id?"ring-4 ring-purple-500 bg-purple-50 scale-[1.02]":"hover:-translate-y-1 hover:shadow-lg"} ${revealedOptionId===option.id?"ring-4 ring-emerald-400":""}`}>
        <button type="button" aria-pressed={selectedOption?.id===option.id} onClick={() => onSelect(option)} className="w-full min-h-32 sm:min-h-44 p-3 sm:p-6 flex justify-center items-center cursor-pointer active:scale-[.98]">
