...
export default function Leaderboard(){
 ...
 const load=useCallback(async()=>{setLoading(true);setError("");try{const requests=[apiRequest(`/leaderboard?period=${period}&page=${page}&limit=20`)];if(page>1)requests.push(apiRequest(`/leaderboard?period=${period}&page=1&limit=3`));if(isSignedIn())requests.push(apiRequest(`/leaderboard/me?period=${period}`));const[data,...extra]=await Promise.all(requests);const top=page>1?extra.shift():data;const mine=isSignedIn()?extra.shift():null;setPlayers(data.players||[]);setLeaders((top.players||[]).slice(0,3));setPagination(data.pagination||{});setMe(mine?.player||null)}catch(e){setError(e.message)}finally{setLoading(false)}},[period,page]);
 useEffect(()=>{load()},[load]);
 useEffect(()=>{const refresh=()=>{if(document.visibilityState!=="hidden")load()};const timer=window.setInterval(refresh,60000);window.addEventListener("focus",refresh);document.addEventListener("visibilitychange",refresh);return()=>{window.clearInterval(timer);window.removeEventListener("focus",refresh);document.removeEventListener("visibilitychange",refresh)}},[load]);
